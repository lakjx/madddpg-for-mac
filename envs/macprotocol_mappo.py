    ...
    def reset(self):
        ...
        for agent in self.agents:
            if agent == 'BS':
                ...
                obs_n[agent_id] = self.get_bs_internal_stat()
            else:
                tar_ue_idx = int(agent.split('_')[1])
                # obs_n.append(self.get_ue_internal_stat(tar_ue_idx))
                agent_id = "agent_" + agent
                obs_n[agent_id] = self.get_ue_internal_stat(tar_ue_idx)
        return obs_n
    def action_interpretation(self,action_n):
        ue_possible_act = list(itertools.product([0,1,2],[0,1]))
        bs_possible_act = list(itertools.product([0,1,2],repeat=self.UE_num))
        ue_act,ucm = [],[]
        for agent_id,act in action_n.items():
            if agent_id == 'agent_BS':
                dcm = bs_possible_act[act]
            else:
                ue_act.append(ue_possible_act[act][0])
                ucm.append(ue_possible_act[act][1])
        return ue_act,ucm,dcm
    def step(self, action_n,UCM=None,DCM=None):
        #MAPPO用
        UE_actions,UCM,DCM = self.action_interpretation(action_n)
        #MADDPG 和 LLMUDTS用
        ...
        for agent in self.agents:
            if agent == 'BS':
                ...
                agent_id = "agent_" + agent
                obs_n[agent_id] = self.get_bs_internal_stat()
                reward_n[agent_id] = self.get_rwd()
                done_n[agent_id] = self.done
            else:
                tar_ue_idx = int(agent.split('_')[1])
                # obs_n.append(self.get_ue_internal_stat(tar_ue_idx))
                agent_id = "agent_" + agent
